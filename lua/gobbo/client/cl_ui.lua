...
function GOBBO.UI:DrawNPCOverhead(entity, options)
	...
	draw.SimpleTextOutlined(options.text, "Gobbo.Nauman.3D2D", 0, options.textOffset, options.color, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 5, options.outlineColor or Color(0, 0, 0))
...
function GOBBO.UI:DrawKeyInteraction(key, text)
	...
	local textMarginLeftX = 12 -- Margin of the text from the box ( left )

	local boxWidth, boxHeight = 36, 36 -- Size of the main box
	...
	draw.RoundedBox(4, xOffset + padding, yOffset + padding, keyBoxWidth, keyBoxHeight, Color(97, 97, 97))

	-- Key Text
	surface.SetFont("Gobbo.UI.Default.Large")
	local textWidth, textHeight = surface.GetTextSize(key)
	local yPosition = yOffset + (boxHeight / 2) - padding -- yOffset + (boxHeight / 2) - (boxHeight - keyBoxHeight) / 2
	...
	if (text) then
		draw.SimpleTextOutlined(text, "Gobbo.UI.Default.Medium", xOffset + boxWidth + textMarginLeftX, yPosition - 1, Color(255, 255, 255), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER, 1, Color(0, 0, 0))
	end
end